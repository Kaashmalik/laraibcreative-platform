      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-gray-50 rounded-lg p-5 border border-gray-200">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex">
              ...
              <span className="font-medium text-gray-900">{formData.customerInfo?.fullName ?? 'N/A'}</span>
            </div>
            <div className="flex">
              <span className="text-gray-600 w-32">Email:</span>
              <span className="font-medium text-gray-900">{formData.customerInfo?.email ?? 'N/A'}</span>
            </div>
            <div className="flex">
              <span className="text-gray-600 w-32">Phone:</span>
              <span className="font-medium text-gray-900">{formData.customerInfo?.phone ?? 'N/A'}</span>
            </div>
            <div className="flex">
              <span className="text-gray-600 w-32">WhatsApp:</span>
              <span className="font-medium text-gray-900 flex items-center gap-1">
                {formData.customerInfo?.whatsapp ?? 'N/A'}
        ...
        <div className="bg-gray-50 rounded-lg p-5 border border-gray-200">
          ...
          <div className="text-sm">
            <p className="font-medium text-gray-900">{formData.shippingAddress?.fullAddress ?? 'N/A'}</p>
            <p className="text-gray-600 mt-1">
              {formData.shippingAddress?.city ? formData.shippingAddress.city.charAt(0).toUpperCase() + formData.shippingAddress.city.slice(1) : 'N/A'}, 
              {' '}{formData.shippingAddress?.province ? formData.shippingAddress.province.charAt(0).toUpperCase() + formData.shippingAddress.province.slice(1) : 'N/A'}
              {' '}- {formData.shippingAddress?.postalCode ?? 'N/A'}
            </p>
        ...
        <div className="bg-gray-50 rounded-lg p-5 border border-gray-200">
          ...
          <div className="space-y-3 text-sm">
            ...
            {formData.payment?.method === 'bank-transfer' && (
              <>
                {formData.payment.receiptImage && (
                  <div>
                    <p className="text-gray-600 mb-2">Receipt:</p>
                    <img 
                      src={formData.payment.receiptImage} 
                      alt="Payment receipt" 
                ...
                <div className="flex">
                  ...
                  <span className="font-medium text-gray-900">{formData.payment.transactionId ?? 'N/A'}</span>
                </div>
                <div className="flex">
                  <span className="text-gray-600 w-32">Transaction Date:</span>
                  <span className="font-medium text-gray-900">
                    {formData.payment.transactionDate
            ...
            {cartData.items.map((item: CartItemDisplay) => (
              <div key={item.id} className="flex gap-4 p-3 bg-white rounded-lg">
                ...
                <div className="flex-1">
                  <h4 className="font-medium text-gray-900">{item.title ?? 'Untitled Product'}</h4>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-5">
          <div className="flex items-start gap-3">
            ...
            <label htmlFor="terms" className="text-sm text-gray-700 cursor-pointer flex-1">
              ...
              <a href="/policies/terms" target="_blank" rel="noopener noreferrer" className="text-purple-600 hover:underline">
                Terms & Conditions
              </a>
              <span className="font-medium"> and </span>
              <a href="/policies/returns" target="_blank" rel="noopener noreferrer" className="text-purple-600 hover:underline">
                Return Policy
              </a>
